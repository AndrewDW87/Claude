...
>(({ className, variant = 'default', ...props }, ref) => (
  <ToastPrimitive.Root
    ...
    className={cn(
      ...
      variant === 'default' && 'border-slate-200 bg-white text-navy',
      variant === 'destructive' && 'border-red-100 bg-red-50 text-red-800',
      variant === 'success' && 'border-green-100 bg-green-50 text-green-800',
      className,
    )}
    {...props}
  />
...
interface ToastState {
  toasts: (ToastProps & { id: string; title?: string; description?: string; variant?: 'default' | 'destructive' | 'success' })[]
}
...
export function toast(opts: { title?: string; description?: string; variant?: 'default' | 'destructive' | 'success' }) {
  ...
  toastState.toasts = [...toastState.toasts, { ...opts, id, open: true }]
  notify({ ...toastState })
  setTimeout(() => {
